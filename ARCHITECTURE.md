...
| S3 upload (frontend) | Backend presigned PUT URL | `/prepare` returns a presigned S3 `PutObject` URL (15 min TTL) signed by the API Lambda role; frontend PUTs directly to that URL with no AWS credentials |
...
```
documents/<user-pool-sub>/<document-id>/<filename>.<ext>
...
Uploads use presigned PUT URLs generated by the API Lambda (no Identity Pool credentials on the frontend). The API Lambda execution role has full bucket read/write. Identity Pool authenticated role retains `GetObject`/`DeleteObject` scoped to `${cognito-identity.amazonaws.com:sub}/*` for any future direct-read needs.
...
1. Browser → POST /single-documents/prepare (or /projects/:id/documents/prepare)
   ...
   API Lambda inserts documents row (status='processing'), generates presigned S3 PutObject URL (15 min),
   returns { docId, uploadKey, uploadUrl }.

2. Browser → S3 PutObject via presigned URL (XHR PUT, no AWS credentials on client)
   s3.PutObject("documents/<user-pool-sub>/<doc-id>/source.<ext>")
...
- Docs bucket: per-user S3 prefix enforced server-side — API Lambda generates presigned URLs only for the authenticated user's own prefix (`documents/{userPoolSub}/`). No client-side AWS credentials involved in uploads.
