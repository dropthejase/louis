...
export default function ProjectChatPage() {
  ...
  const projectMutationSignature = useMemo(() => {
    ...
    return [
      ...
      `edited=${Object.entries(editedPerDoc).map(([k, v]) => `${k}=${v}`).sort().join(',')}`,
    ].join('|');
  }, [messages]);
  ...
  // Load chat title + mark loaded (messages loaded by useAssistantChat hook)
  useEffect(() => {
    if (!chatId) return;
    // Messages are loaded by the hook. We just need the title from chats list.
    setChatLoaded(true);
  }, [chatId]);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      container.scrollTo({ top: el.offsetTop - 16, behavior: 'auto' });
    }, 100);
  ...
  const dismissTabWarning = (documentId: string) => patchTab(documentId, { warning: null });
  ...
  async function handleNewChat() {
    ...
    try {
      ...
      if (id) navigate(`/projects/${projectId}/assistant/chat/${id}`);
    } finally {
  ...
  async function handleDeleteChat() {
    ...
    try {
      ...
      navigate(`/projects/${projectId}?tab=assistant`);
    } finally {
  ...
  async function uploadFiles(files: File[]) {
    ...
    try {
      ...
      setProject((prev) =>
        prev ? { ...prev, documents: [...(prev.documents ?? []), ...uploaded] } : prev,
      );
    } catch (err) {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-8 py-4 shrink-0">
        <div className="flex items-center gap-1.5 text-2xl font-medium font-serif">
          ...
          <span className="text-gray-300">›</span>
          <button
            onClick={() => navigate(`/projects/${projectId}?tab=assistant`)}
            className="text-gray-500 hover:text-gray-700 transition-colors"
          >
            Assistant
          </button>
      ...
      <div className="flex flex-1 min-h-0 border-t border-gray-200 overflow-hidden">
            ...
            >
              ...
              <div
                ...
                onDrop={async (e) => {
                  ...
                  if (docId) { e.stopPropagation(); await handleMoveDoc(docId, null); }
                  else if (folderId) { e.stopPropagation(); await handleMoveFolder(folderId, null); }
                }}
              >
                {explorerDragOver && (
                  <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
                    <p className="text-xs text-blue-500 font-medium">Drop to upload</p>
                  </div>
                )}
                <ProjectExplorer
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r border-gray-200">
          ...
          >
            {tabs.length === 0 ? (
              <span className="px-4 self-center text-xs text-gray-700">Document Viewer</span>
            ) : (
              tabs.map((tab) => {
                ...
                return (
                  <div
                    key={tab.documentId}
                    ref={(el) => { tabItemRefs.current[tab.documentId] = el; }}
                  ...
                  >
                    ...
                    <span className={`text-xs truncate ${isActive ? 'text-gray-900 font-medium' : 'text-gray-500'}`}>
                      {tab.filename}
                    </span>
                    {showVersionBadge && (
                      <span
            ...
            {activeTab ? (
              ...
              <div className="flex items-center justify-center h-full px-8 bg-gray-100">
                <div className="text-center space-y-3">
                  <p className="font-serif text-gray-700 text-xl">Click on a document to display here.</p>
                  <p className="font-serif text-base text-gray-500">
                    Pro tip: Drag a document from the Project Explorer to the Assistant to direct it to read or edit.
                  </p>
                </div>
              </div>
            )}
