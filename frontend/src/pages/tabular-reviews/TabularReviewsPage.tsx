...
export default function TabularReviewsPage() {
  ...
  useEffect(() => { setSelectedIds([]); }, [activeTab, projectFilter]);
  ...
  async function handleDeleteSelected() {
    ...
    if (blocked > 0) {
      setOwnerOnlyAction(`delete ${blocked} of the selected reviews — only the review creator can delete a review`);
    }
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white">
      <div className="flex items-center justify-between px-8 py-4">
        <h1 className="text-2xl font-medium font-serif text-gray-900">Tabular Reviews</h1>
        <div className="flex items-center gap-2">
      ...
      <ToolbarTabs tabs={TABS} active={activeTab} onChange={setActiveTab} actions={toolbarActions} />
            ...
            <div>
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center h-10 pr-8 border-b border-gray-50">
                  ...
                  <div className="w-24 shrink-0"><div className="h-3 w-8 rounded bg-gray-100 animate-pulse" /></div>
                  <div className="w-24 shrink-0"><div className="h-3 w-8 rounded bg-gray-100 animate-pulse" /></div>
                  <div className="w-40 shrink-0"><div className="h-3 w-24 rounded bg-gray-100 animate-pulse" /></div>
            ...
            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
              {activeTab === 'all' && !projectFilter ? (
                <>
                  ...
                  <p className="mt-1 text-xs text-gray-400 max-w-xs text-left">Extract data from documents into tables using AI.</p>
                  <button
                    onClick={() => setNewTROpen(true)}
                    disabled={creating}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-32 shrink-0 text-sm text-gray-500 truncate">
                      {review.created_at ? formatDate(review.created_at) : <span className="text-gray-300">—</span>}
                    </div>
      ...
      <AddNewTRModal open={newTROpen} onClose={() => setNewTROpen(false)} onAdd={handleNewReview} projects={projects} />
      <OwnerOnlyModal open={!!ownerOnlyAction} action={ownerOnlyAction ?? undefined} onClose={() => setOwnerOnlyAction(null)} />
    </div>
  );
}
